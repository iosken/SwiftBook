...
final class TasksViewController: UITableViewController {
    ...
    private var currentTasks: [TaskShadow] {
        data.currentTasks(taskListShadow)
    }
    
    private var completedTasks: [TaskShadow] {
    ...
    override func viewDidLoad() {
        ...
        navigationItem.title = taskListShadow.title
    }
    
    // MARK: - Table View Data Source
    
    override func tableView(_ tableView: UITableView, trailingSwipeActionsConfigurationForRowAt indexPath: IndexPath) -> UISwipeActionsConfiguration? {
        let taskShadow = indexPath.section == 0 ?
        currentTasks[indexPath.row] :
        completedTasks[indexPath.row]
        
        let deleteAction = UIContextualAction(style: .destructive, title: "Delete") { [unowned self] _, _, _ in
            data.delete(taskShadow)
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let result = tableView.dequeueReusableCell(withIdentifier: "TasksCell", for: indexPath)
    ...
    @objc private func addButtonPressed() {
        alert.showAlert(presentIn: self, taskShadow: nil) { [weak self] taskName, taskNote in
            self?.save(taskName: taskName, withNote: taskNote)
        }
    }
...
extension TasksViewController {
    ...
    private func save(taskName: String, withNote taskNote: String) {
        data.save(taskName: taskName, withTaskNote: taskNote, to: taskListShadow) { task in
